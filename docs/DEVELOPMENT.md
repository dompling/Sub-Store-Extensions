...
代码合入 `main` 后不需要手工升版。尚未进入 catalog 的扩展会在下一次 workflow 中自动使用源码版本完成首次发布；已发布扩展会把每个 `extensions/<id>` 与自己的最新 release tag 独立比较。只改一个已发布目录只升级一个扩展，同时存在多个候选时会批量发布。自动增量固定为 patch；minor 或 major 使用手动 workflow dispatch。所有扩展均已发布且目录没有变化时，根目录或文档提交只运行轻量选择器并直接退出。
