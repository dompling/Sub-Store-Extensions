...
const manifest = JSON.parse(readFileSync(
  path.resolve(frontendRoot, '../backend/src/extensions/rule-studio/manifest.json'),
  'utf8',
)) as { version?: unknown };
if (typeof manifest.version !== 'string') {
  throw new Error('Rule-studio manifest requires a release version');
}
...
const extensionGlobal = (id: string) => {
  if (!isExtensionExternal(id)) throw new Error(`Unexpected rule-studio external: ${id}`);
  return sdkGlobal;
};
...
export default defineConfig({
  ...
  build: {
    ...
    lib: {
      entry: path.resolve(frontendRoot, 'src/extensions/rule-studio/runtime-entry.ts'),
      name: 'SubStoreRuleStudioExtension',
      formats: ['iife'],
      fileName: () => 'index.js',
    },
    rollupOptions: {
